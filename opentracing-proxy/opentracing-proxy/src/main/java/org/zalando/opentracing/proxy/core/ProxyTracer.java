...
public final class ProxyTracer implements ForwardingTracer {
    ...
    public <C> void inject(
        ...
        registry.interceptors().injections()
                .intercept(delegate::inject, context, format)
                .inject(context, format, carrier);
    }

    @Override
    public SpanBuilder buildSpan(final String operationName) {
        final SpanBuilder builder = delegate.buildSpan(
    ...
    public ScopeManager scopeManager() {
        return new ProxyScopeManager(delegate.scopeManager(), registry);
    }

    @Override
    public Scope activateSpan(@Nullable final Span span) {
        if (span == null) {
            return delegate.activateSpan(null);
        }

        // we rely on the delegate to support any Span here
        final Scope original = delegate.activateSpan(span);
